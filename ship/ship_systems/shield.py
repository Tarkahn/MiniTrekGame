...
class Shield:
    ...
    def regenerate_integrity(self, delta_time: float, energy_cost: bool = True) -> bool:
        ...
        # Update ship's system integrity display for UI
        if hasattr(self.ship, 'system_integrity'):
            self.ship.system_integrity['shields'] = self.current_integrity
        
        # Only print regeneration message when there's meaningful progress (every 1 point)
        if int(self.current_integrity) != int(self.current_integrity - regen_amount):
            print(f"Shield integrity regenerated. Current: {int(self.current_integrity)}/100")
